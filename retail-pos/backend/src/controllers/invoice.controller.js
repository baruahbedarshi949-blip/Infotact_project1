...
async function buildInvoice(orderId) {
  ...
  // 🔥 FIX: get items from OrderLineItem (NOT order.items)
  const lineItems = await OrderLineItem.find({ orderId: order._id });
...
exports.downloadInvoicePdf = asyncHandler(async (req, res) => {
  ...
  // 🧾 HEADER
  doc.fontSize(20).text('Retail POS Invoice', { align: 'center' });
  doc.moveDown();
  ...
  doc.text(`Date: ${new Date(invoice.issuedAt).toLocaleString()}`);
  doc.moveDown();
  ...
  invoice.items.forEach((item) => {
    doc.text(
      `${item.productTitle} | Qty: ${item.quantity} | ₹${item.unitPrice} | Total: ₹${item.lineTotal}`
    );
  });
  ...
  doc.text(`Discount: ₹${invoice.totals.discountTotal}`);
  doc.text(`Tax: ₹${invoice.totals.taxTotal}`);
  doc.fontSize(14).text(`Grand Total: ₹${invoice.totals.grandTotal}`, {
    align: 'right',
  });
